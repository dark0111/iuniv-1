...
if ($stx)
{
	...
	$sql .= " order by bo_order_search desc, gr_id, bo_table ";
	...
	$search_query = "sfl=".urlencode($sfl)."&search_txt=".urlencode($stx)."&sop=$sop";
	...
	for ($idx=$table_index; $idx<count($search_table); $idx++) 
	{
		...
		for ($i=0; $row=sql_fetch_array($result); $i++) 
		{
			...
			$list[$idx][$i] = $row;
			$list[$idx][$i][href] = "./board/bbs/board.php?bo_table=$search_table[$idx]&wr_id=$row[wr_parent]";
			...
			{
				$content = cut_str($row[wr_content],300,"…");
			...
			$list[$idx][$i][content] = conv_content($row[wr_content], $html);
			if (strstr($sfl, "content"))
				$list[$idx][$i][content] = search_font($stx, $list[$idx][$i][content]);
			$list[$idx][$i][content] = preg_replace("/(\<img )([^\>]*)(\>)/i", "\\1 name='target_resize_image[]' onclick='image_window(this)' style='cursor:pointer;' \\2 \\3", $list[$idx][$i][content]);
	...
	$write_pages = get_paging($config[cf_write_pages], $page, $total_page, "$_SERVER[PHP_SELF]?$search_query&mode=$mode&gr_id=$gr_id&srows=$srows&onetable=$onetable&page=");
...
#######################################################
//주거 파트 시작
if( $search_txt ){//집이름 검색어 검색 조건처리
	$where_str =" and h_nm like '%$search_txt%' ";
}
...
{
	$order_by="h_visit desc,h_nm asc";
...
limit 5 ;
	";
$result_house = sql_query($sql);
$page_total_count = mysql_num_rows($result_house);
...
$search_skin_path = "$g4[path]/skin/search/uta_search_basic";
