...
<head>
	...
	function PrdModify() {
		location.href="_pdt_mdf.php?product_code=<?=$product_code?>&page=<?=$page?>&limit=<?=$limit?>&q=<?=$q?>";
	}

	function PrdDelete() {
	
		if(confirm("본 상품정보를 삭제하시겠습니까?     ")) {
			location.href="_result.php?result=drop&product_code=<?=$product_code?>&page=<?=$page?>&limit=<?=$limit?>&user_type=<?=$LoadProductFetch[user_type]?>&q=<?=$q?>";
		}
	}
	</script>
</head>
