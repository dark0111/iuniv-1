...
# product_contents 에 포함되어 있는 이미지들을 추출하여 변경하고 이미지를 해당 디렉토리에 저장한다. <시작>
	...
	IF(!STRCMP($result, "id")) {
		...
		$ReCheck = "select count(no) as no from $memberSQL where id='$id'";
		$ReCheckSQL = mysql_query($ReCheck, $connection) or die("ReCheck error");
		$ReCheckFetch = mysql_fetch_array($ReCheckSQL);

		if(!strcmp($ReCheckFetch[0],"0")) {
		...
		?>
			...
			<script language="javascript">
			alert("회원등록이 완료되었습니다.         ");
			location.href="./signMemberOk.html";
			</script>
