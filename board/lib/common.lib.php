...
/*************************************************************************
**
**  화면 사이즈및 보드 이미지 등록 사이즈 정의
**
*************************************************************************/
$main_width = 900;
$m_width = 600;
$m_img_width = 600;  // 아키브 이미지 등록시 m_img_width 사이즈 기준 높이는 비율에 맞춰 바뀌고 넚이는 최대 m_img_width크기 
...
{
    global $g4;
	$ex_t=implode('/',array_unique(explode('/',$value)));
	//$ex_t=implode('/',explode('/',$value));
    setcookie($cookie_name, $ex_t, $g4[server_time] + $expire, '/',$g4[cookie_domain]);
}
// 쿠키변수 생성
function set_cookie3($cookie_name, $value, $expire)
{
    global $g4;
	
    setcookie($cookie_name, $value, $g4[server_time] + $expire, '/', $g4[cookie_domain]);
}
// 쿠키변수값 얻음
...
function get_file($bo_table, $wr_id)
{
    ...
    $file["count"] = 0;
    $sql = " select * from $g4[board_file_table] where bo_table = '$bo_table' and wr_id = '$wr_id' order by bf_priority desc,bf_no ";
  //echo"$sql <br>";
	 $no=-1;
...
//카테고리별 글쓰기버튼 표시유무 펑션
function get_cata_write_btn($div_str){
	$cata_sql=" select bo_table, bo_category_list from g4_board where bo_table='plug_board_05' ";

	$result_cata_sql = mysql_query($cata_sql);

	if($row_result_cata_sql = mysql_fetch_array($result_cata_sql)){	
		$bo_category_list = $row_result_cata_sql[bo_category_list];
	}

//echo $bo_category_list;

$arr2 = explode("|",$bo_category_list);

//echo  $bo_category_list."<br>";
	for ($i=0; $i<count($arr2); $i++){
		$exp_str=explode("'",$arr2[$i]);
		$arr2[$i] = $exp_str[0];
		//echo  $arr2[$i]."<br>";
	}
//str_replace("*","",$);
	$rtn_value = 0;
	for ($i=0; $i<count($arr2); $i++){
		//echo "arr2[$i]===".$arr2[$i]."<=>".$div_str."<br>";
		if(substr(trim($arr2[$i]),0,1)=="*"){
			if (trim(str_replace("*","",$arr2[$i]))==trim($div_str)){
...
function get_category_option2($bo_table='',$is_admin,$sca)
{
    global $g4, $board;
    ...
    */
	...
	$arr = explode("|", $cata_str);//위에서 권한 처리와 *붙은 카테고리 관리자전용 처리후 다시 카테고리 분류

    $str = "";
    for ($i=0; $i<count($arr); $i++){
		$div_str = explode("'",$arr[$i]);
        if (trim($arr[$i])){
			if(trim($div_str[0])==$sca){
...
function get_sideview2($mb_id, $name="", $email="", $homepage="")
{
    global $config;
    global $g4;
    ...
    if ($mb_id) {
        ...
        if ($config['cf_use_member_icon']) {
            ...
            if (file_exists($icon_file)) {
                ...
                if ($config['cf_use_member_icon'] == 2) // 회원아이콘+이름
                    $tmp_name = $tmp_name . " <span>$name</span>";
            }
        }
        $title_mb_id = "[$mb_id]";
    } else {
...
//외부로그인 그누보드 연동 파일 인클루드
include_once("$g4[path]/lib/outlogin.lib.php");

function fire_fox_brow_ck(){
