...
$CountProductQuery = "select count(no) as no from $productSQL where user_type='0' ".$qExtendQuery;
...
$ProductLoadQuery = "select * from $productSQL where user_type='0' ".$qExtendQuery." order by no asc limit $first, $limit";
...
for($i=0;$i<$limit;$i++) {
	...
	?>
		...
		<td><?=$POINT?></td>
		<td>
		<?=$ProductLoadFetch[company_code]?>
		<br>
		<span class="date_value"><?=$ProductLoadFetch[date]?></span>
		</td>
		</tr>
		<tr>
		<td colspan=7 align=center height=1 bgcolor="#c4c4c4"><img width=0 height=1></td>
