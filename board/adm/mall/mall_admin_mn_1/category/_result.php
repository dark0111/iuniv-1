...
if(!strcmp($result, "save")) {
	...
	if(!strcmp($checkFetch[0],"0")) {
		...
		$categoryAddQuery .= "scn, wcn, cate_name, ip, date";
		$categoryAddQuery .= ") values (";
		$categoryAddQuery .= "'$scn', '$wcn', '$cate_name', '$REMOTE_ADDR', now()";
		$categoryAddQuery .= ")";
...
<?
	...
	$updateQuery = "update $categorySQL set cate_name='$cate_name', use_yn='$use_yn' where no='$no'";
