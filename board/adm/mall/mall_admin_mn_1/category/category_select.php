...
<head>
	...
	function reForm(ccn, wcn, cateView) {
		...
		// wcn = scn
		// cateView = no
		opener.CateForm.CateValue.value = cateView;
		opener.page.location.href = "../product/_list.php?ccn="+ccn+"&wcn="+wcn;
		this.close();
	}
...
<?
	//2009-12-05 jh추가
	//카테고리 상품을 DB에서 강제 삭제시 갯수 갱신이 안되어서 임시로 업데이트 코드 넣음 
	//추후 개선 필요
	$que1="select count(*) as count,ccn,wcn from mall_goods group by ccn";
	$res1=mysql_query($que1,$connection);
	while($row1=mysql_fetch_array($res1))
	{
		$uque1="update mall_category set cate_info=$row1[count] where no=$row1[ccn] ";
		$res2=mysql_query($uque1,$connection);
	}
...
<td valign=top bgcolor="#ffffff">
	...
	<?
	// 대분류를 불러온다. <시작>
	$categoryLoadQuery = "select no, scn, wcn, cate_name, cate_info, cv from $categorySQL where wcn='1' order by no asc";
	...
	<td height=25>
		...
		<tr>
		<td><img src="../../mall_admin_images/icon/folder<?if(!strcmp($no, $categoryLoadFetch[0])) { print "s";}?>.gif" width="15" height="16" border="0"></td>
		...
		<td style="padding:0 0 0 5"><?if(!strcmp($no, $categoryLoadFetch[0])) {?><a href="javascript:reForm('<?=$categoryLoadFetch[no]?>','<?=$categoryLoadFetch[scn]?>','<?=cate_code_to_list($categoryLoadFetch[no])?>')"><img src="../../mall_admin_images/more/image20.gif" width="33" height="20" border="0"></a><?}?></td>
		</tr>
		...
		if($category_2_CountFetch[0]>0) {
			...
			$category_2_LoadQuery = "select no, scn, wcn, cate_name, cate_info, cv from $categorySQL".$Extend_2_Query." order by no asc";
			...
			for($i2=0;$i2<$category_2_CountFetch[0];$i2++) {
				...
				<td height=25>
					...
					<td><img src="../../mall_admin_images/icon/folder<?if(!strcmp($no, $category_2_LoadFetch[0])) { print "s";}?>.gif" width="15" height="16" border="0"></td>
					...
					<td style="padding:0 0 0 5"><?if(!strcmp($no, $category_2_LoadFetch[0])) {?><a href="javascript:reForm('<?=$category_2_LoadFetch[no]?>','<?=$category_2_LoadFetch[scn]?>','<?=cate_code_to_list($category_2_LoadFetch[no])?>')"><img src="../../mall_admin_images/more/image20.gif" width="33" height="20" border="0"></a><?}?></td>
					</tr>
				...
				if($category_3_CountFetch[0]>0) {
					...
					$category_3_LoadQuery = "select no, scn, wcn, cate_name, cate_info, no, cv from $categorySQL".$Extend_3_Query." order by no asc";
					...
					for($i3=0;$i3<$category_3_CountFetch[0];$i3++) {
						...
						<td height=25>
							...
							<td><img src="../../mall_admin_images/icon/folder<?if(!strcmp($no, $category_3_LoadFetch[0])) { print "s";}?>.gif" width="15" height="16" border="0"></td>
							...
							<td style="padding:0 0 0 5"><?if(!strcmp($no, $category_3_LoadFetch[0])) {?><a href="javascript:reForm('<?=$category_3_LoadFetch[no]?>','<?=$category_3_LoadFetch[scn]?>','<?=cate_code_to_list($category_3_LoadFetch[no])?>')"><img src="../../mall_admin_images/more/image20.gif" width="33" height="20" border="0"></a><?}?></td>
							</tr>
						...
						if($category_4_CountFetch[0]>0) {
							...
							$category_4_LoadQuery = "select no, scn, wcn, cate_name, cate_info from $categorySQL".$Extend_4_Query." order by no asc";
							...
							for($i4=0;$i4<$category_4_CountFetch[0];$i4++) {
								...
								<td height=25>
									...
									<td><img src="../../mall_admin_images/icon/folder<?if(!strcmp($no, $category_4_LoadFetch[0])) { print "s";}?>.gif" width="15" height="16" border="0"></td>
									...
									<td style="padding:0 0 0 5"><?if(!strcmp($no, $category_4_LoadFetch[0])) {?><a href="javascript:reForm('<?=$category_4_LoadFetch[no]?>','<?=$category_4_LoadFetch[scn]?>','<?=cate_code_to_list($category_4_LoadFetch[no])?>')"><img src="../../mall_admin_images/more/image20.gif" width="33" height="20" border="0"></a><?}?></td>
									</tr>
