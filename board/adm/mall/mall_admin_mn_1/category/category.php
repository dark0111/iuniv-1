...
if(1)
{
	...
	<head>
		...
		function addc() {
			parent.page.location.href="_add.php?scn=0&wcn=1";
			location.href="category.php";
		}
		
		function add_parent(mn, loc) {
			parent.page.location.href="./mall_admin_mn_"+mn+"/"+loc+"/_add.php?scn=0&wcn=1";
			location.href=".../../mall_admin_mn_"+mn+"/"+loc+"/category.php";
		}

		function pre() {
			parent.page.location.href="_blank.php";
			location.href="category.php";
		}
		
		function adds(no, scn, wcn) {
			parent.page.location.href="_add.php?no="+no+"&scn="+scn+"&wcn="+wcn;
			location.href="category.php?no="+no+"&wcn="+wcn;
		}
	...
	<body bgcolor="#ffffff" leftmargin=5 scroll="yes" <?if($no) { print "onload='check()'";}?>>
	...
	<!-- 타이틀 <시작> -->
	<table border=0 cellpadding=0 cellspacing=0 width=200 height=25 align=center>
	<tr>
	<td>
		<a href="javascript:pre()" class="title">카테고리</a>
	</td>
	</tr>
	</table>
	<!-- 타이틀 <종료> -->
	...
	<td valign=top bgcolor="#ffffff">
		...
		<?
		// 대분류를 불러온다. <시작>
		$categoryLoadQuery = "select no, scn, wcn, cate_name, cate_info, cv, use_yn from $categorySQL where wcn='1' order by no asc";
		...
		<td height=25>
			...
			if($category_2_CountFetch[0]>0) {
				...
				$category_2_LoadQuery = "select no, scn, wcn, cate_name, cate_info, cv, use_yn from $categorySQL".$Extend_2_Query." order by no asc";
				...
				for($i2=0;$i2<$category_2_CountFetch[0];$i2++) {
					...
					if($category_3_CountFetch[0]>0) {
						...
						$category_3_LoadQuery = "select no, scn, wcn, cate_name, cate_info, cv from $categorySQL".$Extend_3_Query." order by no asc";
						...
						for($i3=0;$i3<$category_3_CountFetch[0];$i3++) {
							...
							<td height=25>
								...
								<tr><td colspan=3><input type="text" name="ct_<?=$category_3_LoadFetch[no]?>"  style="width:1;height:1;border:1 solid;border-color:#ffffff"></td></tr>
								</table>
							...
							if($category_4_CountFetch[0]>0) {
								...
								for($i4=0;$i4<$category_4_CountFetch[0];$i4++) {
									...
									<td height=25>
										...
										<tr><td colspan=3><input type="text" name="ct_<?=$category_4_LoadFetch[no]?>"  style="width:1;height:1;border:1 solid;border-color:#ffffff"></td></tr>
										</table>
