...
$categoryLoadQuery = "select scn, wcn, cate_name, cate_info, cate_info, no from $categorySQL where no='$ccn'";
...
<head>
	...
	function Addprd(){
		if(confirm("'<?=cate_code_to_list($cate_no)?>' 에 \n상품을 등록하시겠습니까?           ")) {
			parent.page.location.href="_add.php?ccn=<?=$ccn?>&wcn=<?=$wcn?>&cate_no=<?=$cate_no?>";
		} else {
...
if($q) {
	$qExtendQuery = " and (cate_no like '%$q%' or product_code like '%$q%' or product_name like '%$q%' or second_product_name like '%$q%')";
} else {
	$qExtendQuery = "";
}
...
for($i=0;$i<$limit;$i++) {
	...
	?>
		<tr align=center>
		<td width=90 style="padding:5 0 5 0"><?=show_product_image("75","$ProductLoadFetch[product_code]");?></td>
...
<tr><td align=right>
	...
	<td><input type="image" src="../../mall_images/more/product_search.gif" width="71" height="21" border="0"></td>
	</form>
	<?if($q) {?>
	<td><a href="_list.php?ccn=<?=$ccn?>&wcn=<?=$wcn?>"><img src="../../mall_images/more/search_end.gif" width="75" height="21" border="0"></a></td>
	<?}?>
	<td><a href="javascript:Addprd()"><img src="../../mall_images/more/product_add.gif" width="96" height="21" border="0"></a></td>
	</tr>
	</table>
</td></tr>
</table>
