...
<br><br>
<?=subtitle("최근 주거comment {$new_comment_rows}건", "./comment_list.php");?>
...
for ($i=0; $row=sql_fetch_array($result); $i++) 
{
    if ($row2['mb_id'] != $row['mb_id'] &&  $row['mb_id'])
    {
        $sql2 = " select mb_id, mb_name, mb_nick, mb_email, mb_homepage, mb_point from $g4[member_table] where mb_id = '$row[mb_id]' ";
        $row2 = sql_fetch($sql2);
		 $mb_nick = get_sideview($row['mb_id'], $row2['mb_nick'], $row2['mb_email'], $row2['mb_homepage']);
	...
	$link1 = "<a href='$IU[url]/index.php?mode=house_view&h_cd=$row[h_id]' target=_blank>";
	$link2 = "</a>";
