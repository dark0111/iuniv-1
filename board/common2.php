...
if (file_exists("$g4[path]/$dbconfig_file")) 
{
   ...
   $connection= $connect_db = sql_connect($mysql_host, $mysql_user, $mysql_password);
...
function select_option($comm_cd,$selected){
	$sql_select_option =" select post_cd, cd, cd_value from room_common_cd where post_cd ='$comm_cd' order by cd asc ";
	$ds_select_option = sql_query($sql_select_option);
	
	$option_string = "";
	for ($i=0; $row = sql_fetch_array($ds_select_option); $i++) {  
		if($selected==$row[cd]){
			$option_string = $option_string."<option value='$row[cd]' selected>$row[cd_value]</option>";
		}else{
			$option_string = $option_string."<option value='$row[cd]'>$row[cd_value]</option>";
		}
	}
...
$IU[house_path]='/home/hosting_users/dark0111/www/sub/house';
//$IU[home_file]= $g4['path'] . "/index.php";
//////////////////////////////////////////////////////
