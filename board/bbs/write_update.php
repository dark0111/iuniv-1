...
if($_POST[wr_key_jh])$_POST[wr_key]=$_POST[wr_key_jh];
if (!$is_member) {
    if ($w=='' || $w=='r') {
...
if (!isset($_POST[wr_subject]) || !trim($_POST[wr_subject])) 
    alert("제목을 입력하여 주십시오."); 
if($_POST[wr_link1]=='http://gosu77.com')
{
	alert("죄송합니다. 접근이 차단된 사용자 이십니다. 관리자에게 문의 해 주세요"); 
}
elseif($_POST[wr_link2]=='http://gosu77.com')
{	
	alert("죄송합니다. 접근이 차단된 사용자 이십니다. 관리자에게 문의 해 주세요"); 
}
elseif($_POST[wr_email]=='stopspem@hanmail.net')
{	
	alert("죄송합니다. 접근이 차단된 사용자 이십니다. 관리자에게 문의 해 주세요"); 
}
elseif($_POST[wr_email]=='http://jogen.y.gp')
{	
	alert("죄송합니다. 접근이 차단된 사용자 이십니다. 관리자에게 문의 해 주세요"); 
}
...
for ($i=0; $i<count($upload); $i++) 
{
    ...
    if ($row[cnt]) 
    {
		...
		{
			$sql = "delete from $g4[board_file_table]  where bo_table = '$bo_table'
                        and wr_id = '$wr_id'
                        and bf_no = '$i' ";
					 sql_query($sql);
		}
