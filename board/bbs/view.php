...
if ($is_admin != "super") {
    // 접근가능 IP
    $bo_accessY_ip = trim($board['bo_accessY_ip']);

    if ($bo_accessY_ip) {
        $Vis_possible_ip = false;
        $Vpattern = explode("\n", $bo_accessY_ip);
        for ($i=0; $i<count($Vpattern); $i++) {
            $Vpattern[$i] = trim($Vpattern[$i]);
            if (empty($Vpattern[$i])) 
                continue;

            //$pat = "/({$Vpattern[$i]})/";
            $Vpattern[$i] = str_replace(".", "\.", $Vpattern[$i]);
            $pat = "/^{$Vpattern[$i]}/";
            $Vis_possible_ip = preg_match($pat, $_SERVER['REMOTE_ADDR']);
            if ($Vis_possible_ip) 
                break;
        }
        if (!$Vis_possible_ip && $member[mb_level] < $board[bo_read_level])
		{
            alert ("접근이 가능하지 않습니다. 해당 게시판 공지사항을 참고 하시기 바랍니다.");
		}

    // 접근차단 IP
	$Vis_intercept_ip = false;
	$Vpattern = explode("\n", trim($board['bo_accessN_ip']));
	for ($i=0; $i<count($Vpattern); $i++) {
		$Vpattern[$i] = trim($Vpattern[$i]);
		if (empty($Vpattern[$i])) 
			continue;
		...
		$Vpattern[$i] = str_replace(".", "\.", $Vpattern[$i]);
		$pat = "/^{$Vpattern[$i]}/";
		$Vis_intercept_ip = preg_match($pat, $_SERVER['REMOTE_ADDR']);
		if ($Vis_intercept_ip) 
		{
			alert ("접근 불가합니다.");
		}
