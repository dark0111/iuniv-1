...
if ($w == "u") 
{
    ...
    {
		$file_length++;
		$i_bf_no=$file[$i][bf_no];
        $row = sql_fetch(" select bf_file, bf_content ,bf_no from $g4[board_file_table] where bo_table = '$bo_table' and wr_id = '$wr_id' and bf_no = '$i_bf_no' ");
		//echo"select bf_file, bf_content ,bf_no from $g4[board_file_table] where bo_table = '$bo_table' and wr_id = '$wr_id' and bf_no = '$i_bf_no' <br>";
        if ($row[bf_file])
        {
		...
		{
			$file_script .= "add_file(\"$row[bf_no]\");\n";
		}
    }
