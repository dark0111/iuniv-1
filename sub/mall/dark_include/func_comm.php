...
******************************************************************************/
	function rg_outlogin($skin='',$ret_url=NULL) {
		global $rs,$_path,$_url,$_mb,$_auth,$dark_define;
		...
		if( $_mb && $_mb['mb_id']!='') 
		{
			...
			$modify_url	= $_url['site']."include/$dark_define[home_file]?code=member_modify&ret_url=".urlencode($ret_url);
			$logout_url	= $_url['site']."include/$dark_define[home_file]?code=member_login&logout&ret_url=".urlencode($ret_url);
//			$leave_url=$_url['member']."mb_leave.php?ret_url=".urlencode($ret_url);
		...
		{
			$login_action = $dark_define['site_url']."/include/$dark_define[home_file]?code=member_login";
			$password_url = $dark_define['site_url']."/include/$dark_define[home_file]?code=member_find_pass&url=".urlencode($ret_url);
			$join_url			= $dark_define['site_url']."/include/$dark_define[home_file]?code=member_join&url=".urlencode($ret_url);
...
******************************************************************************/
	function set_point($mb_num,$po_type,$point,
										 $po_part1='',$po_part2='',$data=NULL,$mb_id=NULL){
		global $dbcon,$_table;
		$rs = new recordset($dbcon);
		
		if($point=='') return false; // 적립사용포인트 없음
		if($mb_num=='' && $mb_id) return false;
	...
	else
	{
		...
		$rs->select();
		$RN=$rs->num_rows();
		if($RN<1) return false; // 회원정보 없음
		if($RN>1) return false; // 회원정보 중복
...
사용법 : dark_mail('수신이메일','제목','내용',['발송자'],['회신메일주소'],
           ['참조'],['숨은참조'])
[내용]
항상 html 방식이다
******************************************************************************/
	// 
	function dark_mail($to,$subject,$message,$from,$return,$cc="ss",$bcc="vv") {
		$header = "";
...
******************************************************************************/
	...
	function cate_code_to_list($ccd){
		global $dbcon,$_table,$_path,$_url,$_mb,$_auth;

		$rs = new recordset($dbcon);
		$rs->clear();
		$rs->set_table($_table[category]);
		$rs->add_where("no='$ccd'");
		$CateRow1=$rs->fetch();
		if($CateRow1) {
			$ccd = $CateRow1[scn];
			$cate_str = $CateRow1[cate_name];
			for($i=0;$i<$CateRow1[wcn];$i++){
				$rs = new recordset($dbcon);
				$rs->clear();
				$rs->set_table($_table[category]);
				$rs->add_where("no='$ccd'");
				$CateRow=$rs->fetch();
				$ccd =  $CateRow[scn];
				if($CateRow[cate_name])
				$cate_str = $CateRow[cate_name]." > ".$cate_str;
			}
			return $cate_str;
		}else{
			return "잘못된 카테고리코드입니다.";
		}
		/*
		$CateQuery1 = "select no, scn, wcn, cate_name, cate_info from $_table[category] where no='$ccd'";
		$CateSQL1 = mysql_query($CateQuery1, $connection) or die("fun->cate_code_to_list CateSQL1 error");
		$CateRow1 = mysql_fetch_array($CateSQL1);
		if($CateRow1) {
			$ccd = $CateRow1[cate_name];
			$cate_str = $CateRow1[cate_name];
			for($i=0;$i<$CateRow1[wcn];$i++){
				$CateQuery = "select no, scn, wcn, cate_name, cate_info from $_table[category] where no='$ccd'";
				$CateSQL[$i] = mysql_query($CateQuery, $connection) or die("fun->33cate_code_to_list CateQuery error");
				$CateRow[$i] = mysql_fetch_array($CateSQL[$i]);
				$cate_str = $CateRow[$i][cate_name]." > ".$cate_str;
			}
			return $cate_str;
		}else{
			return "잘못된 카테고리코드입니다.";
	...
	function common_code_ds($postcd){
		global $connection,$_table;
		$common_cd_sql = "select com_cd, cd_value from $_table[common_cd] where post_cd='$postcd' order by cd_value2 asc";
		$common_cd_ds = mysql_query($common_cd_sql, $connection) or die("공통코드[주문상태코드 로드{".$postcd."}] Query error");
		return $common_cd_ds;
	}

	function common_code_cd_to_val($postcd,$com_cd){
		global $connection,$_table;
		$common_cd_sql = "select com_cd, cd_value from $_table[common_cd] where post_cd='$postcd' and com_cd ='$com_cd'";
		$common_cd_ds = mysql_query($common_cd_sql, $connection) or die("공통코드[코드->값 로드{".$postcd."/".$com_cd."}] Query error");
		$common_cd_row = mysql_fetch_array($common_cd_ds);
		return $common_cd_row[cd_value];
	}
