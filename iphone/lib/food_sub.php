...
if ($_REQUEST["restaurant_nm"]) {
    $restaurant_nm = $_REQUEST['restaurant_nm'];
} else {
    $restaurant_nm = $_REQUEST['restaurant_cd'];
}
...
$restaurant_sql = " SELECT a.wr_id, a.ca_name, a.wr_subject, a.wr_content, a.wr_hit, a.wr_good, a.wr_nogood, a.wr_2, a.wr_4, a.wr_5, a.wr_7, a.wr_8, a.wr_9, a.wr_10, b.bf_file
					...
					WHERE a.wr_subject like '%$restaurant_nm%'
...
$ds_restaurant = mysql_Query($restaurant_sql, $connection) or die('restaurant_sql error');
?>
<div>
    <div class="toolbar">
        <h1>검색어 : <?=cut_str($restaurant_nm,8,'..')?></h1>
        <a href="#" class="button back">Back</a>
    </div>
    <form action="./lib/food_sub.php" method="POST">
		<ul style="padding: 10px 10px 10px 10px; margin: 15px 10px 17px 10px;">
			<input type="text" style="padding: 3px 8px 8px 3px;width:220px" name="restaurant_nm">&nbsp;<a href="#" class="submit"><img align="absmiddle" src="./img/searchButton.png"></a>
		</ul>
    </form>
	<ul class="rounded">
	<?
	if(mysql_num_rows($ds_restaurant)==""){echo"<li>검색 조건에 대한 결과값이 없습니다.</li>";}
	for ($i=0; $row = mysql_fetch_array($ds_restaurant); $i++) {  ?>
		<li class="arrow"><a href="#food_<?=$row[wr_id]?>"><?=$row[wr_subject]?></a> <small class="counter">more</small></li>
	<?}?>
	</ul>
</div>
